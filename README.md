...
## 構成概要
- エントリポイント： cmd/main.go
- 定数： internal/common/constants.go
- 取引所： internal/exchange
  - 現在はbitflyerのみ実装した（internal/exchange/bitflyer）
  - 他の取引所を適用したい場合、「internal/exchange/exchange.go」にあるインターフェース「Exchange」を実装すれば動けるか。configやdbなど「取引所_仮想通貨」で一意の名前を区別するが、テストしたことない
- 指標の計算: internal/indicator
- 取引: internal/trade/trade.go
  - 買い/売り判断: internal/trade/klinestrategies.go
  - 具体的な取引: internal/trade/tradestrategies.go
